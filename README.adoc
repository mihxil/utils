...
* Consistent `equals` and `hashCode`, for the utility function that produce functional implementations. E.g. the the above mentioned `ignoreArg*` and `withArg*` are not simply implemented with lambda's, but with classes with equals and hashcode based on the source functional class implementation.
