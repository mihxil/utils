    ...
    public static class Builder<T>  {
        ...
        public BatchedReceiverSpliterator<T> build() {
            ...
            if (batchType == BatchType.BIFUNCTION && batchSize == null) {
                log.fine("Specified a bifunction, and nobatch size. The batch size is implicitly set to 100");
                batchSize(100);
            }
            if (batchType == BatchType.SUPPLIER && batchSize != null) {
                log.fine("Specified a supplier, and a batch size. The batch size is ignored");
                batchSize(null);
            }
            return _build();
        }
